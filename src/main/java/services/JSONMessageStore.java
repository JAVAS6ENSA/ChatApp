...
public class JSONMessageStore {
    ...
    public void deleteMessage(String user1, String user2, boolean isGroup, long targetTimestamp) {
        // Kept for backward compat. Internally we now soft-delete so
        // the bubble survives as a tombstone — see softDeleteMessage.
        softDeleteMessage(user1, user2, isGroup, targetTimestamp);
    }

    public void softDeleteMessage(String user1, String user2, boolean isGroup, long targetTimestamp) {
        String key = isGroup ? getGroupKey(user2) : getConversationKey(user1, user2);
        List<ChatMessage> list = database.get(key);
        if (list == null) return;
        for (ChatMessage m : list) {
