...
public class ChatController {
    ...
    @FXML private BorderPane mainChatArea;
    ...
    private final Map<Integer, model.Group> userGroups = new HashMap<>();
    private model.Group activeGroup;
    private GroupChatController groupChatCtrl;
    ...
    private void applyButtonIcons() {
        ...
        // Add create group button next to add contact
        Button createGroupBtn = new Button();
        createGroupBtn.setGraphic(makeIcon(IconShape.PLUS, 18, "icon-white"));
        createGroupBtn.getStyleClass().add("btn-icon-green");
        createGroupBtn.setOnAction(e -> onCreateGroup());
        ((HBox)addContactBtn.getParent()).getChildren().add(createGroupBtn);
    }
    ...
    private void loadGroups() {
        if (client != null) client.getGroupList();
    }
    ...
    private void renderContacts(String filter) {
        ...
        if (!userGroups.isEmpty()) {
            Label groupHeader = new Label("GROUPS");
            groupHeader.setStyle("-fx-text-fill: #9ca3af; -fx-font-size: 11px; -fx-font-weight: bold;");
            VBox.setMargin(groupHeader, new Insets(15, 12, 5, 12));
            contactsList.getChildren().add(groupHeader);

            for (model.Group g : userGroups.values()) {
                if (!filter.isEmpty() && !g.getName().toLowerCase().contains(filter.toLowerCase())) continue;

                HBox row = new HBox(12);
                row.getStyleClass().add("contact-row");
                if (activeGroup != null && activeGroup.getId() == g.getId()) {
                    row.getStyleClass().add("contact-row-active");
                }
                row.setAlignment(Pos.CENTER_LEFT);
                row.setPadding(new Insets(10, 14, 10, 14));

                StackPane avatar = buildAvatar("#" + g.getName(), 44); // Use # to distinguish group avatars

                VBox text = new VBox(2);
                Label name = new Label(g.getName());
                name.getStyleClass().add("contact-name");
                Label sub = new Label("Group chat");
                sub.getStyleClass().add("contact-preview");
                ...
                row.setOnMouseClicked(e -> openGroupConversation(g));
                contactsList.getChildren().add(row);
            }
        }
    }
    ...
    private void openConversation(Contact contact) {
        ...
        List<ChatMessage> hist = localStore.getConversation(currentUsername, contact.username, false, contact.username);
        for (ChatMessage m : hist) {
            ...
            addBubble(createMessageNode(m.getContent()), mine,
    ...
    private void sendMessage() {
        ...
        localStore.addMessage(new ChatMessage(currentUsername, activeContact.username, "TEXT", content,
                        time, "sent", mid),
        ...
        pendingTicksByContact
                ...
                .add(tick);
        addBubble(createMessageNode(content), true, time, tick);
        messageInput.clear();
    }
    ...
    private void handleIncoming(String raw) {
        ...
        if ("PRIVATE".equals(type) && p.length == 4) {
            ...
            localStore.addMessage(new ChatMessage(from, currentUsername, "TEXT", text,
                            time, "recv", System.currentTimeMillis()),
                    false, from);
            ...
            if (activeContact != null && activeContact.username.equalsIgnoreCase(from)) {
                addBubble(createMessageNode(text), false, time);
            } else {
        ...
        if ("GROUP_LIST".equals(type) && p.length >= 2) {
            String list = p[1];
            userGroups.clear();
            if (!list.isEmpty()) {
                for (String entry : list.split(",")) {
                    ...
                    if (parts.length == 2) {
                        ...
                        String name = parts[1];
                        userGroups.put(gid, new model.Group(gid, name, 0));
                    }
                }
            }
            renderContacts(searchField.getText());
        ...
        if ("GROUP_MSG".equals(type) && p.length == 4) {
            int gid = Integer.parseInt(p[1]);
            String sender = p[2];
            String text = p[3];
            if (activeGroup != null && activeGroup.getId() == gid && groupChatCtrl != null) {
                groupChatCtrl.handleIncomingMessage(sender, text);
            }
            return;
        }

        if ("GROUP_HISTORY".equals(type) && p.length == 4) {
            int gid = Integer.parseInt(p[1]);
            String sender = p[2];
            String text = p[3];
            if (activeGroup != null && activeGroup.getId() == gid && groupChatCtrl != null) {
                groupChatCtrl.appendHistory(sender, text);
            }
            return;
        }

        if ("GROUP_AUDIO".equals(type) && p.length == 4) {
            int gid = Integer.parseInt(p[1]);
            String sender = p[2];
            String data = p[3];
            if (activeGroup != null && activeGroup.getId() == gid && groupChatCtrl != null) {
                groupChatCtrl.handleIncomingMedia(sender, "AUDIO", data, "");
            }
            return;
        }

        if ("GROUP_FILE".equals(type) && p.length == 5) {
            int gid = Integer.parseInt(p[1]);
            String sender = p[2];
            String fileName = p[3];
            String data = p[4];
            if (activeGroup != null && activeGroup.getId() == gid && groupChatCtrl != null) {
                groupChatCtrl.handleIncomingMedia(sender, "FILE", data, fileName);
            }
            return;
        }
    ...
    private void openGroupConversation(model.Group group) {
        activeContact = null;
        activeGroup = group;
        activeChatTitle.setText(group.getName());
        activeChatStatus.setText("Group Chat");
        activeAvatarBox.getChildren().setAll(buildAvatar("#" + group.getName(), 40));
        
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(getClass().getResource("/group_chat.fxml"));
            Node groupView = loader.load();
            groupChatCtrl = loader.getController();
            
            // For now, assume creator check is not strictly needed for UI toggle here 
            // since GroupChatController handles it.
            groupChatCtrl.initData(client, currentUsername, group.getId(), group.getName(), true);
            
            mainChatArea.setCenter(groupView);
            // Hide the default input bar
            ...
            e.printStackTrace();
            showInfo("Error loading group chat: " + e.getMessage());
        }
        renderContacts(searchField.getText());
    }
    ...
    private void onCreateGroup() {
        ...
        d.setHeaderText("New Group Chat");
        d.setContentText("Group Name:");
        d.showAndWait().ifPresent(name -> {
            if (name.trim().isEmpty()) return;
            TextInputDialog md = new TextInputDialog();
            md.setTitle("Add Members");
            md.setHeaderText("Add members (comma separated)");
            md.setContentText("Usernames:");
            md.showAndWait().ifPresent(members -> {
                client.createGroup(name, List.of(members.split(",")));
            });
        });
    }
